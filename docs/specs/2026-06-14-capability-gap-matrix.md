...
| Agent document map | Shipped | `include_document_map`; links pages, elements, selectable text-layer and metadata coverage, chunks, layout diagnostics, safety findings, accessibility report routing, OCR evidence, visual-region candidate indexes, visual enrichment indexes, and page geometry in one agent-ready contract. |
...
| Accessibility report | Shipped | `include_accessibility_report`; deterministic tagged-PDF coverage, tag-to-visible-content coverage, structure tree, heading, image, form, link, permission, mark-info, issue type/severity, document-vs-page issue, page-grade, and affected-page signals. Does not claim PDF/UA certification. |
...
3. Harden the agent document map as the SSOT for pages, elements, text-layer
   coverage, chunks, layout, safety, accessibility routing, page geometry, and
