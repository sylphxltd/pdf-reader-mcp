...
| Table quality diagnostics | Shipped | Completeness, non-empty cell ratio, cell bounding-box coverage, inferred-cell ratio, row alignment, row spacing consistency, missing-cell count, inferred merged-cell candidates, incomplete-geometry warnings, and repeated-header continuation candidates. |
...
| PDF trust report | Shipped | `include_trust_report`; consolidates content safety, layout uncertainty, sparse/scanned-page, table quality, external-link, and unsafe-link signals with page-level routing guidance. |
...
| Quality eval and benchmark harness | Shipped | Regression evals cover semantic chunks, table order, table evidence coverage, renderers, safety findings, inspection routing, recursive reading order, visual-region normalization, caption-derived formula/chart candidate routing, document-twin visual enrichment fusion, document-map text-layer coverage, caption-to-evidence links, search evidence, hidden-text safety routing, and unsafe-link trust routing. `bun run benchmark:quality` publishes deterministic quality gates for Agent Document Twin semantics, inspection tool routing, real PDF document signals, real PDF reading order, deterministic table cell evidence coverage, runtime-generated scanned-PDF OCR pipeline routing, OCR normalization, OCR-derived table extraction, caption-derived visual candidate routing, command/HTTP visual-region normalization, evidence search, and AI-safety trust-report hidden-text/unsafe-link routing. `bun run benchmark:providers` reports installed-provider certification profiles for OCR text-layer word boxes and visual table/formula/chart/figure/image-description crop evidence when providers are configured. |
...
| Formula extraction | Shipped | `analyze_regions` can normalize LaTeX, MathML, AsciiMath, text, confidence, and provenance from a configured provider; `read_pdf` can opt into `include_visual_enrichments`, derive bounded formula regions from semantic captions when no image object exists, and attach formula evidence to the document twin. Accuracy depends on the configured local engine. |
| Chart/image descriptions | Shipped | `analyze_regions` can normalize chart data points, axes, series, figure, and image-description provider output; `read_pdf` can opt into `include_visual_enrichments`, route table/image elements plus caption-derived chart/figure/diagram regions, and attach visual evidence to the document twin. Accuracy depends on the configured local engine. |
