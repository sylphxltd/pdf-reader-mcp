...
| Quality eval and benchmark harness | Shipped | Regression evals cover semantic chunks, table order, renderers, safety findings, inspection routing, recursive reading order, visual-region normalization, document-twin visual enrichment fusion, document-map text-layer coverage, caption-to-evidence links, search evidence, and unsafe-link trust routing. `bun run benchmark:quality` publishes deterministic quality gates for Agent Document Twin semantics, inspection tool routing, real PDF document signals, real PDF reading order, runtime-generated scanned-PDF OCR pipeline routing, OCR normalization, command/HTTP visual-region normalization, evidence search, and AI-safety trust-report unsafe-link routing. `bun run benchmark:providers` reports installed-provider certification profiles for OCR text-layer word boxes and visual table/formula/chart/figure/image-description crop evidence when providers are configured. |
