...
export const readPdfArgsSchema = object({
  ...
  include_document_map: optional(
    bool(
      description(
        'Include an agent-ready document map that links pages, elements, text-layer coverage, chunks, layout diagnostics, safety findings, routing signals, and page geometry without embedding image bytes in JSON.'
      )
    )
  ),
  ...
  include_trust_report: optional(
    bool(
      description(
        'Include a PDF trust report that consolidates content safety, visual-spoofing, tiny/off-page text, layout uncertainty, sparse/scanned-page, table-quality, external-link, unsafe-link, category-count, and page-risk signals for agent routing.'
      )
    )
  ),
