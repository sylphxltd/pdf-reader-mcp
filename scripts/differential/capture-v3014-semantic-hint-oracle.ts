...
const runnerSource = join(scriptDir, 'v3014-semantic-hint-baseline-runner.ts');
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-semantic-hint-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, oracle.baseline.commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const runner = join(worktree, 'v3014-semantic-hint-baseline-runner.ts');
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 semantic-hint oracle differs from executable baseline');
  } else {
    console.log(`v3.0.14 semantic-hint oracle replay: OK (${Object.keys(expectations).length} cases)`);
  }
} finally {
