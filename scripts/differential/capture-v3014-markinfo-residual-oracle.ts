...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-markinfo-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-markinfo-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-markinfo-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-markinfo-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  for (const [name, path] of Object.entries(fixtures)) {
    copyFileSync(path, join(relocatedFixtureDir, name));
  }
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-markinfo-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('markinfo residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_markinfo_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: Object.fromEntries(
        Object.entries(fixtures).map(([name, path]) => [name, sha256(readFileSync(path))])
      ),
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 markinfo residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 markinfo residual oracle matches detached executable baseline');
  }
} finally {
