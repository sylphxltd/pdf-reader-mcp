...
export const canonicalMetadataPresenceResidualResult = (response: unknown): Json => {
  ...
  const results = array(record(resultsSource, 'results source').results, 'results').map(
    (entry, index) => {
      ...
      const projectedData: Record<string, Json> = {
        ...
        has_info: Object.hasOwn(data, 'info'),
        has_metadata: Object.hasOwn(data, 'metadata'),
      };
      // Claim exact metadata payload when present. TS/pdfjs-dist Node yields {}.
      if (Object.hasOwn(data, 'metadata')) {
        ...
        if (
          ...
          throw new Error(
            `results[${String(index)}].data.metadata must be an empty object in this residual`
          );
        }
