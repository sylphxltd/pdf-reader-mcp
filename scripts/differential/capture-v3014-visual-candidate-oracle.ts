...
const sourcePaths = [
  'src/handlers/readPdf.ts', 'src/pdf/autoReadPolicy.ts', 'src/pdf/readCoordinator.ts',
  'src/pdf/visualEnrichment.ts', 'src/pdf/semanticPatterns.ts', 'src/pdf/documentModel.ts',
  'src/pdf/tableExtractor.ts', 'src/pdf/documentMap.ts', 'src/pdf/regionAnalysis.ts',
  'src/pdf/parser.ts', 'src/pdf/loader.ts', 'src/types/pdf/region-analysis.ts',
...
run('bun', ['scripts/differential/generate-v3014-visual-candidate-fixtures.ts'], repoRoot);
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-visual-candidate-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-visual-candidate-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-visual-candidate-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(run('bun', [join(worktree, 'v3014-visual-candidate-baseline-runner.ts'), corpusPath, fixtureDir], worktree, true));
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_visual_candidate_oracle',
    baseline: {
      ...
      generatorSha256: sha256(readFileSync(generatorPath)), corpusSha256: sha256(readFileSync(corpusPath)),
      fixtureManifestSha256: sha256(readFileSync(manifestPath)),
      fixtureSha256: Object.fromEntries(manifest.fixtures.map((fixture) => [fixture.path, fixture.sha256])),
      envelope: corpus.envelope, nonclaims: corpus.nonclaims,
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 visual-candidate oracle differs from executable baseline');
  } else {
    console.log(`v3.0.14 visual-candidate oracle replay: OK (${String(Object.keys(expectations).length)} cases)`);
  }
} finally {
