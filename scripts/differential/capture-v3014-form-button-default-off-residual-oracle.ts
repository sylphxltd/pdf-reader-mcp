...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-form-button-default-off-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-form-button-default-off-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-form-button-default-off-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-form-button-default-off-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(radioApFixture, join(relocatedFixtureDir, 'v3014-form-radio-ap-default-off-v1.pdf'));
  copyFileSync(checkboxNoApFixture, join(relocatedFixtureDir, 'v3014-form-checkbox-noap-default-null-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-form-button-default-off-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('form button-default-off residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_form_button_default_off_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        ...
        'v3014-form-radio-ap-default-off-v1.pdf': sha256(readFileSync(radioApFixture)),
        'v3014-form-checkbox-noap-default-null-v1.pdf': sha256(readFileSync(checkboxNoApFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 form button-default-off residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 form button-default-off residual oracle matches detached executable baseline');
  }
} finally {
