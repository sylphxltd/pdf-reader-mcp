...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-outline-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-attachment-invalid-uf-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-attachment-invalid-uf-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-attachment-invalid-uf-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-outline-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-attachment-invalid-uf-valid-v1.pdf'), readFileSync(validFixture));
    writeFileSync(join(relocated, 'v3014-attachment-invalid-uf-nofallback-v1.pdf'), readFileSync(nofallbackFixture));
    writeFileSync(join(relocated, 'v3014-attachment-invalid-uf-fonly-v1.pdf'), readFileSync(fonlyFixture));
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-attachment-invalid-uf-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('outline attachment invalid-uf residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_attachment_invalid_uf_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-attachment-invalid-uf-valid-v1.pdf': sha256(readFileSync(validFixture)),
        'v3014-attachment-invalid-uf-nofallback-v1.pdf': sha256(readFileSync(nofallbackFixture)),
        'v3014-attachment-invalid-uf-fonly-v1.pdf': sha256(readFileSync(fonlyFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 outline attachment invalid-uf residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 outline attachment invalid-uf residual oracle matches detached executable baseline');
  }
} finally {
