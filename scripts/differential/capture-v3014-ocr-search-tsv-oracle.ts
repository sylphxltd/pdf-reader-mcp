...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-ocr-search-tsv-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-ocr-search-tsv-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-ocr-search-tsv-projection.ts'), readFileSync(projectionPath));
  writeFileSync(join(worktree, 'v3014-ocr-search-projection.ts'), readFileSync(sharedProjectionPath));
  writeFileSync(join(worktree, 'reference-ocr-search-tsv-provider.ts'), readFileSync(providerPath));
  const detachedProvider = join(worktree, 'reference-ocr-search-tsv-provider.ts');
  const expectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-ocr-search-tsv-baseline-runner.ts'),
        corpusPath,
        fixtureDir,
        detachedProvider,
      ],
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('ocr-search tsv projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_ocr_search_tsv_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 ocr-search tsv oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 ocr-search tsv oracle matches detached executable baseline');
  }
} finally {
