...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-document-ast-visual-fusion-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const detachedRunner = join(worktree, 'v3014-document-ast-visual-fusion-baseline-runner.ts');
  const detachedProjection = join(worktree, 'v3014-document-ast-visual-fusion-projection.ts');
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('document-ast-visual-fusion projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_document_ast_visual_fusion_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 document-ast-visual-fusion oracle differs from detached executable baseline');
  } else {
    console.log(
      `v3.0.14 document-ast-visual-fusion oracle replay: OK (${String(Object.keys(expectations).length)} cases)`
    );
  }
} finally {
