...
const projectAnnotation = (value: unknown, context: string): Json => {
  ...
  if (Object.hasOwn(annotation, 'bounding_box') && annotation.bounding_box != null) {
    projected.bounding_box = projectBox(annotation.bounding_box, `${context}.bounding_box`);
  }
  if (Object.hasOwn(annotation, 'id')) projected.id = string(annotation.id, `${context}.id`);
  if (Object.hasOwn(annotation, 'contents')) projected.contents = string(annotation.contents, `${context}.contents`);
  if (Object.hasOwn(annotation, 'title')) projected.title = string(annotation.title, `${context}.title`);
  return projected;
};

export const canonicalPopupZeroSizeResidualResult = (response: unknown): Json => {
  ...
  const resultsSource = Object.hasOwn(payload, 'results')
    ...
    : Object.hasOwn(payload, 'data')
      ...
      : payload;
  const results = array(record(resultsSource, 'results source').results, 'results').map((entry, index) => {
    ...
    if (!result.success) {
      if (Object.hasOwn(result, 'error')) projected.error = string(result.error, `results[${String(index)}].error`);
      return projected;
    }
    const data = record(result.data, `results[${String(index)}].data`);
    const groups = array(data.annotations, `results[${String(index)}].data.annotations`).map((group, groupIndex) => {
      const pageGroup = record(group, `annotations[${String(groupIndex)}]`);
      return {
        page: integer(pageGroup.page, `annotations[${String(groupIndex)}].page`),
        annotations: array(pageGroup.annotations, `annotations[${String(groupIndex)}].annotations`).map(
          (annotation, annotationIndex) =>
            projectAnnotation(annotation, `annotations[${String(groupIndex)}].annotations[${String(annotationIndex)}]`)
        ),
      };
    });
    projected.data = {
  ...
  return {
    profile: string(record(resultsSource, 'profile source').profile ?? 'pdf_read_results', 'profile'),
    results,
  };
};
