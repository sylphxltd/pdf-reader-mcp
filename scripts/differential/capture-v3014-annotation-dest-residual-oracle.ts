...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-annotation-dest-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-annotation-dest-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-annotation-dest-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-annotation-dest-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-annotation-dest-residual-relocated-'));
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-dest-fit-v1.pdf'),
      readFileSync(fitFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-dest-xyz-v1.pdf'),
      readFileSync(xyzFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-annotation-dest-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('annotation dest residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_annotation_dest_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-dest-fit-v1.pdf': sha256(readFileSync(fitFixture)),
        'v3014-annotation-dest-xyz-v1.pdf': sha256(readFileSync(xyzFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 annotation dest residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 annotation dest residual oracle matches detached executable baseline');
  }
} finally {
