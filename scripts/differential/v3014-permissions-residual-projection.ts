...
export const canonicalPermissionsResidualResult = (response: unknown): Json => {
  ...
  const results = array(record(resultsSource, 'results source').results, 'results').map(
    (entry, index) => {
      ...
      if (Object.hasOwn(data, 'permissions')) {
        projectedData.permissions = array(
          data.permissions,
          `results[${String(index)}].data.permissions`
        ).map((label, labelIndex) =>
          string(label, `results[${String(index)}].data.permissions[${String(labelIndex)}]`)
        );
      }
