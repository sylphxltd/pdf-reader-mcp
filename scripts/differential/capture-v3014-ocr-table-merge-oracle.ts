...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-ocr-table-merge-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-ocr-table-merge-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-ocr-table-merge-projection.ts'), readFileSync(projectionPath));
  writeFileSync(join(worktree, 'reference-ocr-table-merge-provider.ts'), readFileSync(providerPath));
  const detachedProvider = join(worktree, 'reference-ocr-table-merge-provider.ts');
  const expectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-ocr-table-merge-baseline-runner.ts'),
        corpusPath,
        fixtureDir,
        detachedProvider,
      ],
  ...
  copyFileSync(selectableFixture, join(relocatedFixtureDir, 'v3014-selectable-table-v1.pdf'));
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('ocr-table-merge projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_ocr_table_merge_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-selectable-table-v1.pdf': sha256(readFileSync(selectableFixture)),
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 ocr-table-merge oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 ocr-table-merge oracle matches detached executable baseline');
  }
} finally {
