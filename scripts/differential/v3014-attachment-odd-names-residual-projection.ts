...
export const canonicalAttachmentOddNamesResidualResult = (response: unknown): Json => {
  ...
  const results = array(record(resultsSource, 'results source').results, 'results').map(
    (entry, index) => {
      ...
      projected.data = {
        ...
        attachments: array(data.attachments, `results[${String(index)}].data.attachments`).map(
          projectAttachment
        ),
      };
      return projected;
