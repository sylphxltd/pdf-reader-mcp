...
const projectAnnotation = (entry: unknown, index: number): Json => {
  ...
  if (Object.hasOwn(annotation, 'dest')) {
    projected.dest = array(annotation.dest, `annotations[${String(index)}].dest`).map(projectDestPart);
  }
...
export const canonicalAnnotationDestResidualResult = (response: unknown): Json => {
