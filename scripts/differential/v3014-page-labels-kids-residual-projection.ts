...
export const canonicalPageLabelsKidsResidualResult = (response: unknown): Json => {
  ...
  const results = array(record(resultsSource, 'results source').results, 'results').map(
    (entry, index) => {
      ...
      // pdf.js/TS omit page_labels when absent; claim presence/absence exactly.
      if (Object.hasOwn(data, 'page_labels')) {
        projectedData.page_labels = array(
          data.page_labels,
          `results[${String(index)}].data.page_labels`
        ).map((label, labelIndex) =>
          string(label, `results[${String(index)}].data.page_labels[${String(labelIndex)}]`)
        );
      }
