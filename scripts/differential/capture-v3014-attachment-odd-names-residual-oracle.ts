...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-attachment-odd-names-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-attachment-odd-names-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-attachment-odd-names-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-attachment-odd-names-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(orphanFixture, join(relocatedFixtureDir, 'v3014-attachment-odd-names-v1.pdf'));
  copyFileSync(pairFixture, join(relocatedFixtureDir, 'v3014-attachment-odd-names-pair-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-attachment-odd-names-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('attachment odd-names residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_attachment_odd_names_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-attachment-odd-names-v1.pdf': sha256(readFileSync(orphanFixture)),
        'v3014-attachment-odd-names-pair-v1.pdf': sha256(readFileSync(pairFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 attachment odd-names residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 attachment odd-names residual oracle matches detached executable baseline');
  }
} finally {
