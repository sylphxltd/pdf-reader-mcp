...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-encrypt-filter-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-encrypt-filter-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-encrypt-filter-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-encrypt-filter-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-encrypt-filter-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-permissions-print-copy-fill-a11y-v1.pdf'), readFileSync(encryptedFixture));
    writeFileSync(join(relocated, 'v3014-permissions-none-v1.pdf'), readFileSync(unencryptedFixture));
    ...
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'encrypt filter residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_encrypt_filter_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-permissions-print-copy-fill-a11y-v1.pdf': sha256(readFileSync(encryptedFixture)),
        'v3014-permissions-none-v1.pdf': sha256(readFileSync(unencryptedFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 encrypt filter residual oracle differs from detached baseline'
    );
  } else {
    console.log('v3.0.14 encrypt filter residual oracle matches detached executable baseline');
  }
} finally {
