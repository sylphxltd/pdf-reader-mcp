...
import { canonicalPopupAnnotationResidualResult } from './v3014-popup-annotation-residual-projection.ts';
const [corpusPath, fixtureDir] = process.argv.slice(2);
if (!corpusPath || !fixtureDir) throw new Error('usage: v3014-popup-annotation-residual-baseline-runner <corpus.json> <fixtureDir>');
...
const invoke = async (entry: Case): Promise<unknown> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => { reject(new Error(`TS popup residual timed out: ${stderr.slice(-2000)}`)); }, 120_000);
      pending.set(id, (value) => { clearTimeout(timer); resolve(value); });
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params })}\n`);
    });
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-popup-annotation-residual-baseline', version: '1' } });
