...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-page-geometry-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-page-geometry-userunit-multipage-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-page-geometry-userunit-multipage-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-page-geometry-userunit-multipage-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-page-geometry-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-page-geometry-userunit-pages-v1.pdf'), readFileSync(userunitPagesFixture));
    writeFileSync(join(relocated, 'v3014-page-geometry-multi-page-v1.pdf'), readFileSync(multiPageFixture));
    writeFileSync(join(relocated, 'v3014-page-geometry-bleed-ignore-v1.pdf'), readFileSync(bleedIgnoreFixture));
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-page-geometry-userunit-multipage-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('page geometry userunit-multipage residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_page_geometry_userunit_multipage_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-page-geometry-userunit-pages-v1.pdf': sha256(readFileSync(userunitPagesFixture)),
        'v3014-page-geometry-multi-page-v1.pdf': sha256(readFileSync(multiPageFixture)),
        'v3014-page-geometry-bleed-ignore-v1.pdf': sha256(readFileSync(bleedIgnoreFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 page geometry userunit-multipage residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 page geometry userunit-multipage residual oracle matches detached executable baseline');
  }
} finally {
