...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-text-inverted-rect-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-text-inverted-rect-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-text-inverted-rect-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run('bun', [join(worktree, 'v3014-text-inverted-rect-residual-baseline-runner.ts'), corpusPath, fixtureDir], worktree, true)
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-text-inverted-rect-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-annotation-text-inverted-v1.pdf'), readFileSync(invertedFixture));
    writeFileSync(join(relocated, 'v3014-annotation-text-noap-v1.pdf'), readFileSync(noApFixture));
    const relocatedExpectations = JSON.parse(
      run('bun', [join(worktree, 'v3014-text-inverted-rect-residual-baseline-runner.ts'), corpusPath, relocated], worktree, true)
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('text inverted-rect residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_text_inverted_rect_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-text-inverted-v1.pdf': sha256(readFileSync(invertedFixture)),
        'v3014-annotation-text-noap-v1.pdf': sha256(readFileSync(noApFixture))
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 text inverted-rect residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 text inverted-rect residual oracle matches detached executable baseline');
  }
} finally {
