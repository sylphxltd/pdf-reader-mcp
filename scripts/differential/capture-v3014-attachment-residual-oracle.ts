...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-attachment-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-attachment-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-attachment-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-attachment-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(kidsFixture, join(relocatedFixtureDir, 'v3014-attachment-kids-v1.pdf'));
  copyFileSync(filenameFixture, join(relocatedFixtureDir, 'v3014-attachment-filename-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-attachment-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('attachment residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_attachment_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-attachment-kids-v1.pdf': sha256(readFileSync(kidsFixture)),
        'v3014-attachment-filename-v1.pdf': sha256(readFileSync(filenameFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 attachment residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 attachment residual oracle matches detached executable baseline');
  }
} finally {
