...
const runnerSource = join(scriptDir, 'v3014-citation-chunk-baseline-runner.ts');
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-citation-chunk-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, oracle.baseline.commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const runner = join(worktree, 'v3014-citation-chunk-baseline-runner.ts');
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 citation-chunk oracle differs from executable baseline');
  } else {
    console.log(
      `v3.0.14 citation-chunk oracle replay: OK (${Object.keys(expectations).length} cases)`
    );
  }
} finally {
