...
const polylineApAsOnFixture = join(fixtureDir, 'v3014-annotation-underline-quad-noap-v1.pdf');
...
try {
  ...
  writeFileSync(
    join(worktree, 'v3014-annotation-text-markup-quadpoints-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-annotation-text-markup-quadpoints-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-annotation-text-markup-quadpoints-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-underline-quad-noap-v1.pdf'),
      readFileSync(polylineApAsOnFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-squiggly-quad-noap-v1.pdf'),
      readFileSync(lineApAsOnFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-strikeout-quad-noap-v1.pdf'),
      readFileSync(lineApAsInvalidFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-annotation-text-markup-quadpoints-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'annotation AP text-markup quadpoints residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_annotation_text_markup_quadpoints_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-underline-quad-noap-v1.pdf': sha256(readFileSync(polylineApAsOnFixture)),
        'v3014-annotation-squiggly-quad-noap-v1.pdf': sha256(readFileSync(lineApAsOnFixture)),
        'v3014-annotation-strikeout-quad-noap-v1.pdf': sha256(readFileSync(lineApAsInvalidFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 annotation AP text-markup quadpoints residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 annotation AP text-markup quadpoints residual oracle matches detached executable baseline'
    );
  }
} finally {
