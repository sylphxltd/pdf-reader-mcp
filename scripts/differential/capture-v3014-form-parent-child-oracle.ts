...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-form-parent-child-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-form-parent-child-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-form-parent-child-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-form-parent-child-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(childFixture, join(relocatedFixtureDir, 'v3014-form-parent-child-v1.pdf'));
  copyFileSync(readonlyFixture, join(relocatedFixtureDir, 'v3014-form-parent-readonly-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-form-parent-child-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('form parent-child projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_form_parent_child_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-form-parent-child-v1.pdf': sha256(readFileSync(childFixture)),
        'v3014-form-parent-readonly-v1.pdf': sha256(readFileSync(readonlyFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 form parent-child oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 form parent-child oracle matches detached executable baseline');
  }
} finally {
