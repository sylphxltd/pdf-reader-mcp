...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-read-ocr-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const detachedRunner = join(worktree, 'v3014-read-ocr-baseline-runner.ts');
  const detachedProjection = join(worktree, 'v3014-read-ocr-projection.ts');
  const detachedProvider = join(worktree, 'reference-ocr-provider.ts');
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('read-ocr projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_read_ocr_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 read-ocr oracle differs from detached executable baseline');
  } else {
    console.log(
      `v3.0.14 read-ocr oracle replay: OK (${String(Object.keys(expectations).length)} cases)`
    );
  }
} finally {
