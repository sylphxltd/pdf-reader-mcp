...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-ink-annotation-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-ink-annotation-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-ink-annotation-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-ink-annotation-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-ink-annotation-relocated-'));
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-ink-l-bbox-v1.pdf'),
      readFileSync(inkFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-ink-multistroke-v1.pdf'),
      readFileSync(multiStrokeFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-ink-border2-v1.pdf'),
      readFileSync(border2Fixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-ink-annotation-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'ink annotation residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_ink_annotation_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-ink-l-bbox-v1.pdf': sha256(readFileSync(inkFixture)),
        'v3014-annotation-ink-multistroke-v1.pdf': sha256(readFileSync(multiStrokeFixture)),
        'v3014-annotation-ink-border2-v1.pdf': sha256(readFileSync(border2Fixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 ink annotation residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 ink annotation residual oracle matches detached executable baseline'
    );
  }
} finally {
