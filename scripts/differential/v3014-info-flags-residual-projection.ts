...
const projectInfo = (value: unknown): Json => {
  ...
  for (const key of INFO_KEYS) {
    ...
    if (entry === null) {
      ...
      projected[key] = boolean(entry, `info.${key}`);
    } else if (typeof entry === 'string') {
      projected[key] = string(entry, `info.${key}`);
    } else {
      throw new Error(`info.${key} has unsupported type`);
    }
  }
  return projected;
};

export const canonicalInfoFlagsResidualResult = (response: unknown): Json => {
