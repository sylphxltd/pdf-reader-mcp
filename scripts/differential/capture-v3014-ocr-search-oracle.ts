...
const sourcePaths = ['src/index.ts', 'src/mcp.ts', 'src/handlers/searchPdf.ts', 'src/schemas/searchPdf.ts', 'src/schemas/readPdf.ts', 'src/pdf/search.ts', 'src/pdf/ocr.ts', 'src/pdf/renderer.ts', 'src/pdf/extractor.ts', 'src/pdf/parser.ts', 'src/pdf/loader.ts', 'src/utils/geometry.ts', 'src/types/pdf.ts', 'src/types/pdf/search.ts', 'src/types/pdf/ocr.ts', 'src/types/pdf/source.ts'];
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-ocr-search-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const detachedRunner = join(worktree, 'v3014-ocr-search-baseline-runner.ts');
  const detachedProjection = join(worktree, 'v3014-ocr-search-projection.ts');
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_ocr_search_oracle',
    baseline: {
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 OCR-search oracle differs from detached executable baseline');
  } else {
    console.log(`v3.0.14 OCR-search oracle replay: OK (${String(Object.keys(expectations).length)} cases)`);
  }
} finally {
