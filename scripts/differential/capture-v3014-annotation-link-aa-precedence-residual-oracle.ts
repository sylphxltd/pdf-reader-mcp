...
try {
  ...
  writeFileSync(
    join(worktree, 'v3014-annotation-link-aa-precedence-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-annotation-link-aa-precedence-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-annotation-link-aa-precedence-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-link-aa-e-ignored-v1.pdf'),
      readFileSync(aaEFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-link-dest-over-aa-v1.pdf'),
      readFileSync(destAaFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-link-aa-d-over-u-v1.pdf'),
      readFileSync(aaDOverUFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-annotation-link-aa-precedence-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'annotation AP link-aa-precedence residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_annotation_link_aa_precedence_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-link-aa-e-ignored-v1.pdf': sha256(readFileSync(aaEFixture)),
        'v3014-annotation-link-dest-over-aa-v1.pdf': sha256(readFileSync(destAaFixture)),
        'v3014-annotation-link-aa-d-over-u-v1.pdf': sha256(readFileSync(aaDOverUFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 annotation AP link-aa-precedence residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 annotation AP link-aa-precedence residual oracle matches detached executable baseline'
    );
  }
} finally {
