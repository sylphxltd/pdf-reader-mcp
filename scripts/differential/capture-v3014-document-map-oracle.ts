...
const runnerSource = join(scriptDir, 'v3014-document-map-baseline-runner.ts');
const projectionSource = join(scriptDir, 'v3014-document-map-projection.ts');
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-document-map-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, oracle.baseline.commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const runner = join(worktree, 'v3014-document-map-baseline-runner.ts');
  const projection = join(worktree, 'v3014-document-map-projection.ts');
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 document-map oracle differs from executable baseline');
  } else {
    console.log(`v3.0.14 document-map oracle replay: OK (${Object.keys(expectations).length} cases)`);
  }
} finally {
