...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-info-trapped-custom-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-info-trapped-custom-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-info-trapped-custom-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-info-trapped-custom-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-info-trapped-custom-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-info-trapped-true-v1.pdf'), readFileSync(trappedTrueFixture));
    writeFileSync(join(relocated, 'v3014-info-trapped-false-v1.pdf'), readFileSync(trappedFalseFixture));
    writeFileSync(join(relocated, 'v3014-info-custom-mixed-v1.pdf'), readFileSync(customMixedFixture));
    ...
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'info trapped-custom residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_info_trapped_custom_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-info-trapped-true-v1.pdf': sha256(readFileSync(trappedTrueFixture)),
        'v3014-info-trapped-false-v1.pdf': sha256(readFileSync(trappedFalseFixture)),
        'v3014-info-custom-mixed-v1.pdf': sha256(readFileSync(customMixedFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 info trapped-custom residual oracle differs from detached baseline'
    );
  } else {
    console.log('v3.0.14 info trapped-custom residual oracle matches detached executable baseline');
  }
} finally {
