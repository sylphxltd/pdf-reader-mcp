...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-permissions-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-permissions-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-permissions-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-permissions-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-permissions-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-permissions-print-copy-fill-a11y-v1.pdf'), readFileSync(printCopyFixture));
    writeFileSync(join(relocated, 'v3014-permissions-modify-annotate-assemble-v1.pdf'), readFileSync(modifyFixture));
    writeFileSync(join(relocated, 'v3014-permissions-print-hq-v1.pdf'), readFileSync(printHqFixture));
    writeFileSync(join(relocated, 'v3014-permissions-none-v1.pdf'), readFileSync(noneFixture));
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-permissions-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('permissions residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_permissions_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-permissions-print-copy-fill-a11y-v1.pdf': sha256(readFileSync(printCopyFixture)),
        'v3014-permissions-modify-annotate-assemble-v1.pdf': sha256(readFileSync(modifyFixture)),
        'v3014-permissions-print-hq-v1.pdf': sha256(readFileSync(printHqFixture)),
        'v3014-permissions-none-v1.pdf': sha256(readFileSync(noneFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 permissions residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 permissions residual oracle matches detached executable baseline');
  }
} finally {
