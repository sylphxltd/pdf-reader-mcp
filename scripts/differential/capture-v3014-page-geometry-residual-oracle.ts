...
try {
  ...
  writeFileSync(join(worktree, 'v3014-page-geometry-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-page-geometry-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-page-geometry-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  try {
    writeFileSync(join(relocated, 'v3014-page-geometry-rotate-userunit-v1.pdf'), readFileSync(rotateFixture));
    writeFileSync(join(relocated, 'v3014-page-geometry-default-v1.pdf'), readFileSync(defaultFixture));
    writeFileSync(join(relocated, 'v3014-page-geometry-inverted-mediabox-v1.pdf'), readFileSync(invertedFixture));
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-page-geometry-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error('page geometry residual projection depends on checkout or fixture-root path');
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_page_geometry_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-page-geometry-rotate-userunit-v1.pdf': sha256(readFileSync(rotateFixture)),
        'v3014-page-geometry-default-v1.pdf': sha256(readFileSync(defaultFixture)),
        'v3014-page-geometry-inverted-mediabox-v1.pdf': sha256(readFileSync(invertedFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 page geometry residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 page geometry residual oracle matches detached executable baseline');
  }
} finally {
