...
import { canonicalVisualFusionResult } from './v3014-visual-fusion-projection.ts';
...
const invoke = async (
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-visual-fusion-baseline', version: '1' },
    });
    child.stdin.write(
...
for (const entry of corpus.cases) {
  ...
  expectations[entry.id] = canonicalVisualFusionResult(response, invocations);
}
