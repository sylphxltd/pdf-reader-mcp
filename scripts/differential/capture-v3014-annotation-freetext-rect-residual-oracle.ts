...
try {
  ...
  writeFileSync(
    join(worktree, 'v3014-annotation-freetext-rect-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-annotation-freetext-rect-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-annotation-freetext-rect-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-freetext-inverted-v1.pdf'),
      readFileSync(invertedFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-freetext-normal-v1.pdf'),
      readFileSync(normalFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-freetext-zero-width-v1.pdf'),
      readFileSync(zeroFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-annotation-freetext-rect-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'annotation AP freetext-rect residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_annotation_freetext_rect_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-freetext-inverted-v1.pdf': sha256(readFileSync(invertedFixture)),
        'v3014-annotation-freetext-normal-v1.pdf': sha256(readFileSync(normalFixture)),
        'v3014-annotation-freetext-zero-width-v1.pdf': sha256(readFileSync(zeroFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 annotation AP freetext-rect residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 annotation AP freetext-rect residual oracle matches detached executable baseline'
    );
  }
} finally {
