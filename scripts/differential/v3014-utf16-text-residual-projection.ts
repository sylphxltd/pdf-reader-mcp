...
const projectAnnotation = (value: unknown, context: string): Json => {
  ...
  if (Object.hasOwn(annotation, 'contents')) {
    projected.contents = string(annotation.contents, `${context}.contents`);
  }
...
const projectInfo = (value: unknown, context: string): Json => {
  ...
  const projected: Record<string, Json> = {};
  for (const key of ['Title', 'Author', 'Producer'] as const) {
    if (Object.hasOwn(info, key) && info[key] != null) {
      projected[key] = string(info[key], `${context}.${key}`);
    }
  }
  return projected;
};

const projectOutline = (value: unknown, context: string): Json => {
  const item = record(value, context);
  return {
    title: string(item.title, `${context}.title`),
    bold: boolean(item.bold, `${context}.bold`),
    italic: boolean(item.italic, `${context}.italic`),
  };
};

export const canonicalUtf16TextResidualResult = (response: unknown): Json => {
  ...
  const results = array(record(resultsSource, 'results source').results, 'results').map(
    (entry, index) => {
      ...
      const dataOut: Record<string, Json> = {
        num_pages: integer(data.num_pages, `results[${String(index)}].data.num_pages`),
      };
      if (Object.hasOwn(data, 'annotations')) {
        dataOut.annotations = array(
          data.annotations,
      ...
      if (Object.hasOwn(data, 'outline')) {
        dataOut.outline = array(data.outline, `results[${String(index)}].data.outline`).map(
          (item, itemIndex) => projectOutline(item, `outline[${String(itemIndex)}]`)
        );
      }
      if (Object.hasOwn(data, 'info')) {
        dataOut.info = projectInfo(data.info, `results[${String(index)}].data.info`);
      }
      projected.data = dataOut;
      return projected;
    }
  );
  return {
