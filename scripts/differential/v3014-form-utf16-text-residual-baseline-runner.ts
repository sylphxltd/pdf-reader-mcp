...
import { canonicalFormUtf16TextResidualResult } from './v3014-form-utf16-text-residual-projection.ts';

const [corpusPath, fixtureDir] = process.argv.slice(2);
if (!corpusPath || !fixtureDir) {
  throw new Error('usage: v3014-form-utf16-text-residual-baseline-runner <corpus.json> <fixtureDir>');
}
...
const invoke = async (entry: Case): Promise<unknown> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`TS form utf16-text residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-utf16-text-residual-baseline', version: '1' },
    });
    child.stdin.write(
