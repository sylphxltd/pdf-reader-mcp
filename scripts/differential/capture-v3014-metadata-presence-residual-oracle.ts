...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-metadata-presence-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-metadata-presence-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-metadata-presence-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-metadata-presence-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-metadata-presence-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-metadata-absent-v1.pdf'), readFileSync(absentFixture));
    writeFileSync(join(relocated, 'v3014-metadata-present-v1.pdf'), readFileSync(presentFixture));
    ...
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'metadata presence residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_metadata_presence_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-metadata-absent-v1.pdf': sha256(readFileSync(absentFixture)),
        'v3014-metadata-present-v1.pdf': sha256(readFileSync(presentFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 metadata presence residual oracle differs from detached baseline'
    );
  } else {
    console.log('v3.0.14 metadata presence residual oracle matches detached executable baseline');
  }
} finally {
