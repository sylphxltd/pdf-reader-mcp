...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-ocr-tsv-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-ocr-tsv-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-ocr-tsv-projection.ts'), readFileSync(projectionPath));
  writeFileSync(join(worktree, 'v3014-read-ocr-projection.ts'), readFileSync(sharedProjectionPath));
  writeFileSync(join(worktree, 'reference-ocr-tsv-provider.ts'), readFileSync(providerPath));
  const detachedProvider = join(worktree, 'reference-ocr-tsv-provider.ts');
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-ocr-tsv-baseline-runner.ts'), corpusPath, fixtureDir, detachedProvider],
      worktree,
      true
    )
  );
  ...
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-ocr-tsv-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
        detachedProvider,
      ],
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('ocr-tsv projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_ocr_tsv_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 ocr-tsv oracle differs from detached executable baseline');
  } else {
    console.log('v3.0.14 ocr-tsv oracle matches detached executable baseline');
  }
} finally {
