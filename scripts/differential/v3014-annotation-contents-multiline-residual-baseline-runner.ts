...
import { canonicalAnnotationContentsMultilineResidualResult } from './v3014-annotation-contents-multiline-residual-projection.ts';

const [corpusPath, fixtureDir] = process.argv.slice(2);
if (!corpusPath || !fixtureDir) {
  throw new Error(
    'usage: v3014-annotation-contents-multiline-residual-baseline-runner <corpus.json> <fixtureDir>'
  );
}
...
const invoke = async (entry: Case): Promise<unknown> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`TS annotation AP contents-multiline residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-contents-multiline-residual-baseline', version: '1' },
    });
    child.stdin.write(
