...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-polyline-polygon-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-polyline-polygon-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-polyline-polygon-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-polyline-polygon-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-polyline-polygon-relocated-'));
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-polyline-l-bbox-v1.pdf'),
      readFileSync(polylineFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-polygon-l-bbox-v1.pdf'),
      readFileSync(polygonFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-polyline-border2-v1.pdf'),
      readFileSync(border2Fixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-polyline-polygon-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'polyline/polygon residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_polyline_polygon_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-polyline-l-bbox-v1.pdf': sha256(readFileSync(polylineFixture)),
        'v3014-annotation-polygon-l-bbox-v1.pdf': sha256(readFileSync(polygonFixture)),
        'v3014-annotation-polyline-border2-v1.pdf': sha256(readFileSync(border2Fixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 polyline/polygon residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 polyline/polygon residual oracle matches detached executable baseline'
    );
  }
} finally {
