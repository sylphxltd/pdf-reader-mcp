...
import { canonicalReadOcrResult } from './v3014-read-ocr-residual-projection.ts';

const [corpusPath, fixtureDir, providerPath] = process.argv.slice(2);
if (!corpusPath || !fixtureDir || !providerPath) {
  throw new Error(
    'usage: v3014-read-ocr-residual-baseline-runner <corpus.json> <fixtureDir> <provider.ts>'
  );
}
...
const invoke = async (entry: Case): Promise<unknown> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`TS residual read-ocr request timed out: ${stderr.slice(-2000)}`));
      }, 180_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-read-ocr-residual-baseline', version: '1' },
    });
    child.stdin.write(
