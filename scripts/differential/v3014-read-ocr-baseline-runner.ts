...
import { canonicalReadOcrResult } from './v3014-read-ocr-projection.ts';
...
const invoke = async (entry: (typeof corpus.cases)[number]): Promise<unknown> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`TS read-ocr request timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-read-ocr-baseline', version: '1' },
    });
    child.stdin.write(
