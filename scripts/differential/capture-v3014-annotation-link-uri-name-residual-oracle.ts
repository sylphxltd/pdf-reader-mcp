...
try {
  ...
  writeFileSync(
    join(worktree, 'v3014-annotation-link-uri-name-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-annotation-link-uri-name-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-annotation-link-uri-name-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-link-uri-name-v1.pdf'),
      readFileSync(nameFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-link-uri-javascript-v1.pdf'),
      readFileSync(jsFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-link-uri-relative-v1.pdf'),
      readFileSync(relFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-annotation-link-uri-name-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'annotation AP link-uri-name residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_annotation_link_uri_name_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-annotation-link-uri-name-v1.pdf': sha256(readFileSync(nameFixture)),
        'v3014-annotation-link-uri-javascript-v1.pdf': sha256(readFileSync(jsFixture)),
        'v3014-annotation-link-uri-relative-v1.pdf': sha256(readFileSync(relFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 annotation AP link-uri-name residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 annotation AP link-uri-name residual oracle matches detached executable baseline'
    );
  }
} finally {
