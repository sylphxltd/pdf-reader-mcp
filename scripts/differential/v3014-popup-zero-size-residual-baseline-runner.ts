...
import { canonicalPopupZeroSizeResidualResult } from './v3014-popup-zero-size-residual-projection.ts';
const [corpusPath, fixtureDir] = process.argv.slice(2);
if (!corpusPath || !fixtureDir) throw new Error('usage: v3014-popup-zero-size-residual-baseline-runner <corpus.json> <fixtureDir>');
...
const invoke = async (entry: Case): Promise<unknown> => {
  const child = spawn(process.execPath, ['src/index.ts'], {
    cwd: process.cwd(), stdio: ['pipe', 'pipe', 'pipe'], env: { ...process.env, MCP_TRANSPORT: 'stdio' },
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => { reject(new Error(`TS popup zero-size residual timed out: ${stderr.slice(-2000)}`)); }, 120_000);
      pending.set(id, (value) => { clearTimeout(timer); resolve(value); });
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params })}\n`);
    });
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-popup-zero-size-residual-baseline', version: '1' } });
