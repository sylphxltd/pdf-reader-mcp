...
const FIXTURES = [
  'v3014-annotation-link-aa-e-ignored-v1.pdf',
  'v3014-annotation-link-dest-over-aa-v1.pdf',
  'v3014-annotation-link-aa-d-over-u-v1.pdf',
] as const;
...
const projectAnnotation = (value: unknown, context: string): Json => {
  ...
  if (Object.hasOwn(annotation, 'url')) {
    projected.url = string(annotation.url, `${context}.url`);
  }
  if (Object.hasOwn(annotation, 'dest')) {
    projected.dest = annotation.dest as Json;
  }
...
export const canonicalAnnotationLinkAaPrecedenceResidualResult = (response: unknown): Json => {
