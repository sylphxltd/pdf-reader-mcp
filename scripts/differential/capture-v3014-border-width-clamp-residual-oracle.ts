...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-border-width-clamp-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-border-width-clamp-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-border-width-clamp-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-border-width-clamp-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-border-width-clamp-relocated-'));
  try {
    writeFileSync(
      join(relocated, 'v3014-annotation-polyline-clamp-w2-v1.pdf'),
      readFileSync(polylineClampFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-line-clamp-w2-v1.pdf'),
      readFileSync(lineClampFixture)
    );
    writeFileSync(
      join(relocated, 'v3014-annotation-ink-clamp-w2-v1.pdf'),
      readFileSync(inkClampFixture)
    );
    const relocatedExpectations = JSON.parse(
      run(
        'bun',
        [join(worktree, 'v3014-border-width-clamp-residual-baseline-runner.ts'), corpusPath, relocated],
        worktree,
        true
      )
    );
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'border-width clamp residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    ...
    baseline: {
      ...
      fixtureSha256: {
        'v3014-annotation-polyline-clamp-w2-v1.pdf': sha256(readFileSync(polylineClampFixture)),
        'v3014-annotation-line-clamp-w2-v1.pdf': sha256(readFileSync(lineClampFixture)),
        'v3014-annotation-ink-clamp-w2-v1.pdf': sha256(readFileSync(inkClampFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 border-width clamp residual oracle differs from detached baseline'
    );
  } else {
    console.log(
      'v3.0.14 border-width clamp residual oracle matches detached executable baseline'
    );
  }
} finally {
