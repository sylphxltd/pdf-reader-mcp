...
const projectInfo = (value: unknown, context: string): Json => {
  ...
  const encrypt = info.EncryptFilterName;
  let encryptFilterName: Json = null;
  if (encrypt === null || encrypt === undefined) {
    encryptFilterName = null;
  } else if (typeof encrypt === 'string') {
    encryptFilterName = string(encrypt, `${context}.EncryptFilterName`);
  } else {
    throw new Error(`${context}.EncryptFilterName must be string or null`);
  }
  return {
...
export const canonicalEncryptFilterResidualResult = (response: unknown): Json => {
