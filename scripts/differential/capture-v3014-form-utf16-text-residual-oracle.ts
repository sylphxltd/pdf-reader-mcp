...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-form-utf16-text-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-form-utf16-text-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-form-utf16-text-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-form-utf16-text-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(utf16Fixture, join(relocatedFixtureDir, 'v3014-form-utf16-odd-v1.pdf'));
  copyFileSync(utf8Fixture, join(relocatedFixtureDir, 'v3014-form-utf8-bom-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-form-utf16-text-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('form utf16-text residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_form_utf16_text_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-form-utf16-odd-v1.pdf': sha256(readFileSync(utf16Fixture)),
        'v3014-form-utf8-bom-v1.pdf': sha256(readFileSync(utf8Fixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 form utf16-text residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 form utf16-text residual oracle matches detached executable baseline');
  }
} finally {
