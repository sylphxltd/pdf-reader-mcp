...
run('bun', ['scripts/differential/generate-v3014-lowercase-index-fixture.ts'], repoRoot);
...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-lowercase-index-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  const runner = join(worktree, 'v3014-lowercase-index-baseline-runner.ts');
  const projection = join(worktree, 'v3014-lowercase-index-projection.ts');
  ...
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as { fixture: { sha256: string } };
  const corpus = JSON.parse(readFileSync(corpusPath, 'utf8')) as { localeContract: unknown };
  const oracle = {
    schemaVersion: 1, profile: 'pdf_reader_v3014_lowercase_index_oracle',
    baseline: {
      ...
      fixtureManifestSha256: sha256(readFileSync(manifestPath)), fixtureSha256: manifest.fixture.sha256,
      localeContract: corpus.localeContract,
  ...
  if (refresh) {
    ...
    throw new Error('stored v3.0.14 lowercase-index oracle differs from executable baseline');
  } else {
    console.log(`v3.0.14 lowercase-index oracle replay: OK (${String(Object.keys(expectations).length)} cases)`);
  }
} finally {
