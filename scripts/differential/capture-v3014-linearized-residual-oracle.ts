...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-linearized-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(
    join(worktree, 'v3014-linearized-residual-baseline-runner.ts'),
    readFileSync(runnerPath)
  );
  writeFileSync(
    join(worktree, 'v3014-linearized-residual-projection.ts'),
    readFileSync(projectionPath)
  );
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-linearized-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  const relocated = mkdtempSync(join(tmpdir(), 'pdf-reader-linearized-relocated-'));
  try {
    writeFileSync(join(relocated, 'v3014-info-linearized-valid-v1.pdf'), readFileSync(validFixture));
    writeFileSync(join(relocated, 'v3014-info-linearized-spurious-v1.pdf'), readFileSync(spuriousFixture));
    writeFileSync(join(relocated, 'v3014-info-linearized-absent-v1.pdf'), readFileSync(absentFixture));
    ...
    if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
      throw new Error(
        'linearized residual projection depends on checkout or fixture-root path'
      );
    }
  } finally {
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_linearized_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-info-linearized-valid-v1.pdf': sha256(readFileSync(validFixture)),
        'v3014-info-linearized-spurious-v1.pdf': sha256(readFileSync(spuriousFixture)),
        'v3014-info-linearized-absent-v1.pdf': sha256(readFileSync(absentFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error(
      'stored v3.0.14 linearized residual oracle differs from detached baseline'
    );
  } else {
    console.log('v3.0.14 linearized residual oracle matches detached executable baseline');
  }
} finally {
