...
const worktree = mkdtempSync(join(tmpdir(), 'pdf-reader-v3014-form-button-array-residual-'));
try {
  run('git', ['worktree', 'add', '--detach', worktree, commit], repoRoot);
  run('bun', ['install', '--frozen-lockfile'], worktree);
  writeFileSync(join(worktree, 'v3014-form-button-array-residual-baseline-runner.ts'), readFileSync(runnerPath));
  writeFileSync(join(worktree, 'v3014-form-button-array-residual-projection.ts'), readFileSync(projectionPath));
  const expectations = JSON.parse(
    run(
      'bun',
      [join(worktree, 'v3014-form-button-array-residual-baseline-runner.ts'), corpusPath, fixtureDir],
      worktree,
      true
    )
  );
  ...
  copyFileSync(arrayFixture, join(relocatedFixtureDir, 'v3014-form-button-array-v-v1.pdf'));
  copyFileSync(dvFixture, join(relocatedFixtureDir, 'v3014-form-button-array-dv-v1.pdf'));
  const relocatedExpectations = JSON.parse(
    run(
      'bun',
      [
        join(worktree, 'v3014-form-button-array-residual-baseline-runner.ts'),
        corpusPath,
        relocatedFixtureDir,
      ],
      worktree,
  ...
  if (JSON.stringify(relocatedExpectations) !== JSON.stringify(expectations)) {
    throw new Error('form button-array residual projection depends on checkout or fixture-root path');
  }
  ...
  const oracle = {
    schemaVersion: 1,
    profile: 'pdf_reader_v3014_form_button_array_residual_oracle',
    baseline: {
      tag: 'v3.0.14',
      commit,
      tree,
      ...
      fixtureSha256: {
        'v3014-form-button-array-v-v1.pdf': sha256(readFileSync(arrayFixture)),
        'v3014-form-button-array-dv-v1.pdf': sha256(readFileSync(dvFixture)),
      },
  ...
  if (refresh || !existsSync(oraclePath)) {
    ...
    throw new Error('stored v3.0.14 form button-array residual oracle differs from detached baseline');
  } else {
    console.log('v3.0.14 form button-array residual oracle matches detached executable baseline');
  }
} finally {
