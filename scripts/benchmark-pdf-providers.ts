...
const runRegionAnalysisProviderBenchmark = async (): Promise<ProviderBenchmarkResult> => {
  ...
  if (status.readiness === 'not_configured') {
    return {
      ...
      duration_ms: round(performance.now() - start),
      message:
        'Set MCP_PDF_REGION_ANALYSIS_COMMAND, MCP_PDF_REGION_ANALYSIS_HTTP_URL, or MCP_PDF_REGION_ANALYSIS_PRESET=ollama to benchmark a configured visual-region provider.',
      provider_status: {
